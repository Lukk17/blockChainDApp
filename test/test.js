...
contract('dBank', ([deployer, user]) => {
    ...
    //       expect(await token.name()).to.be.eq('LukkCoin')
    //     })
    //
    //     it('checking token symbol', async () => {
    //       expect(await token.symbol()).to.be.eq('LKC')
    //     })
    //
    ...
    describe('testing deposit...', () => {
        ...
        describe('failure', () => {
            it('depositing should be rejected due to too small amount', async () => {
                await dbank.deposit({value: 10 ** 15, from: user}).should.be.rejectedWith(EVM_REVERT)
            })
            it('depositing should be rejected due to already deposited', async () => {
